...
export default function showUpdateItemDialog(props) {
  console.log(props)
  props.dispatch(showDialog({ content: <UpdateItemDialog {...props} />, maxWidth: 'md' }))
}

// eslint-disable-next-line react-refresh/only-export-components
function UpdateItemDialog(props) {
  const { t } = useTranslation()

  const { dispatch, database, reload, type, keyColumn, defaultModel } = props
  ...
  return (
    <>
      <DialogTitle>{t('DIALOG_UPDATE')}</DialogTitle>
      <DialogContent>
        <Typography variant="h6">{t('DIALOG_UPDATE_TEXT')}</Typography>
